...
\begin{align}
Cov(Y_{ij}, Y_{i'j}) &= \mathbb E \left[ (Y_{ij} - \mathbb E [Y_{ij}])(Y_{i'j} - \mathbb E [Y_{i'j}])\right] \\
&= \mathbb E \left[ (\mu + \tau_i + \beta_j + \epsilon_{ij} - (\mu + \tau_i))(\mu + \tau_{i'} + \beta_j + \epsilon_{i'j} - (\mu + \tau_{i'}))\right] \\
&= \mathbb E \left[ (\beta_j + \epsilon_{ij})(\beta_j + \epsilon_{i'j}) \right] \\
&= \mathbb E \left[ (\beta_j^2 + \beta_j \epsilon_{i'j} + \beta_j \epsilon_{ij} + \epsilon_{ij}\epsilon_{i'j}) \right] \\
&= 0
\end{align}
$$
...
The mean $\mu$ and each treatment effect must be held constant. It must also hold that $\mathbb E[\beta_j] = 0$ and $\mathbb E[\epsilon_{ij}] = 0$ for all $i,j$.
...
T_neg <- twin_df %>% filter(signs == '-') %>% select(ranks) %>% sum(); T_neg
```
...
```{r}
wilcox <- wilcox.test(twin_df$a, twin_df$b, 
                      paired = T, alternative = 'greater');wilcox
```
...
```{r}
T_pos <- twin_df %>% filter(signs == '+') %>% select(ranks) %>% sum(); T_pos
```
